...
{ 
    ...
    if (status == NNSE_STATUS_FAILURE) {
        while (1)
            example_status = NNSE_STATUS_INIT_FAILED; // hang
    }
        ...
        int16_t *pcm_output)
{
    ...
    */
    ...
    for (int i =0; i < FEATURE_NUM_MFC; i++)
    {
        ...
        model.model_input[0]->data.i16[i] =  input;
    }
    ...
    if (invoke_status != kTfLiteOk) {
        while (1)
        {
            example_status = NNSE_STATUS_FAILURE; // invoke failed, so hang
        }
    }
