...
static void
UnityPrintDecimalAndNumberWithLeadingZeros(UNITY_INT32 fraction_part, UNITY_INT32 divisor) {
    UNITY_OUTPUT_CHAR('.');
    while (divisor > 0) {
...
void UnityPrintFloat(const UNITY_DOUBLE input_number) {
    ...
    else if (number <= (UNITY_DOUBLE)0.0000005 && number > 0)
        UnityPrint("0.000000...");                      /* Small number */
    else if (number < (UNITY_DOUBLE)4294967295.9999995) /* Rounded result fits in 32 bits, "%.6f"
                                                           format */
    {
        ...
        ROUND_TIES_TO_EVEN(
            number, fraction_part, (number - (UNITY_DOUBLE)integer_part) * 1000000.0);
...
static int UnityIsOneArrayNull(
    UNITY_INTERNAL_PTR expected, UNITY_INTERNAL_PTR actual, const UNITY_LINE_TYPE lineNumber,
    const char *msg) {
    if (expected == actual)
    ...
    while (elements--) {
        ...
        if (expect_val != actual_val) {
            if (style & UNITY_DISPLAY_RANGE_UINT &&
                length < sizeof(expect_val)) { /* For UINT, remove sign extension (padding 1's) from
                                                  signed type casts above */
                UNITY_INT mask = 1;
    ...
    do {
        ...
        if (Unity.CurrentTestFailed) {
            ...
            UnityPrintExpectedAndActualStrings(
                (const char *)(expected[j]), (const char *)(actual[j]));
            UnityAddMsgIfSpecified(msg);
            UNITY_FAIL_AND_BAIL;
        }
...
int UnityEnd(void) {
    ...
    UNITY_PRINT_EOL();
    AM_UPDATE_TEST_RESULT(
        (UNITY_INT)(Unity.NumberOfTests), (UNITY_INT)(Unity.TestFailures),
