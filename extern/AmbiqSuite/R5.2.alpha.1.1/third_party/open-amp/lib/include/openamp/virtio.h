...
/* Ambiq: in general the driver or device support can be enabled in application
 * layer. In ambiqsuite the OpenAMP is used as driver (host) role by default.
 * If device (remote) role is needed, we need to define WITH_VIRTIO_DEVICE in
 * the application to enable device role support.
 */
#define VIRTIO_DRIVER_SUPPORT    1
#ifdef WITH_VIRTIO_DEVICE
#define VIRTIO_DEVICE_SUPPORT    1
#else
#define VIRTIO_DEVICE_SUPPORT    0
#endif /* WITH_VIRTIO_DEVICE */
