...
private:
    ...
    erpc_status_t ns_rpc_data_sendBlockToPC_shim(erpc::Codec * codec, erpc::MessageBufferFactory *messageFactory, uint32_t sequence);

    /*! @brief Server shim for ns_rpc_data_fetchBlockFromPC of evb_to_pc interface. */
    erpc_status_t ns_rpc_data_fetchBlockFromPC_shim(erpc::Codec * codec, erpc::MessageBufferFactory *messageFactory, uint32_t sequence);

    /*! @brief Server shim for ns_rpc_data_computeOnPC of evb_to_pc interface. */
    erpc_status_t ns_rpc_data_computeOnPC_shim(erpc::Codec * codec, erpc::MessageBufferFactory *messageFactory, uint32_t sequence);

    /*! @brief Server shim for ns_rpc_data_remotePrintOnPC of evb_to_pc interface. */
    erpc_status_t ns_rpc_data_remotePrintOnPC_shim(erpc::Codec * codec, erpc::MessageBufferFactory *messageFactory, uint32_t sequence);
};
...
erpc_service_t create_evb_to_pc_service(void);

#if ERPC_ALLOCATION_POLICY == ERPC_ALLOCATION_POLICY_DYNAMIC
void destroy_evb_to_pc_service(erpc_service_t service);
#elif ERPC_ALLOCATION_POLICY == ERPC_ALLOCATION_POLICY_STATIC
void destroy_evb_to_pc_service(void);
#else
