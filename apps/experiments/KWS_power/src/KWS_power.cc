...
#if 0 == 1
#include "KWS_model.h"
#include "KWS_platform.h"         // platform macros
#include "KWS_tensors.h"          // tensor sizes
#include "KWS_context.h"          // model context
#include "KWS_power_aot_api.h"
#include "KWS_power_aot_example_tensors.h"
...
#include "KWS_power_example_tensors.h"
#include "ns_model.h"
#endif
                         ...
                         void *user_data)
{
    ...
    if (state == KWS_op_state_run_started) {
        ns_pmu_accm_op_begin(aot_matrix, op);
    }

    if (state == KWS_op_state_run_finished) {
        ns_pmu_accm_op_end(aot_matrix, op);

    }
}

static KWS_model_context_t model = {
    // .input_data = {KWS_power_example_input_tensors},
    // .output_data = {KWS_power_output_tensors},
    #if 1 == 0
...
int main(void) {
    ...
    // memcpy(model.outputs[0].data, KWS_outputs, sizeof(KWS_outputs));
    // memcpy(model.input_len, KWS_inputs_len, sizeof(KWS_inputs_0_size));
    // memcpy(model.output_len, KWS_outputs_len, sizeof(KWS_outputs_0_size));
    ns_lp_printf("Initializing AOT model...\n");
    int status = KWS_model_init(&model); // model init with minimal defaults
    memcpy(model.inputs[0].data, KWS_power_example_input_tensors, sizeof(KWS_power_example_input_tensors));
    ...
    // }
#else
    int status = KWS_power_minimal_init(&model); // model init with minimal defaults
#endif
    if (status == KWS_power_STATUS_FAILURE) {
        ns_lp_printf("TFLM Model init failed.\n");
        while (1);
        example_status = KWS_power_STATUS_INIT_FAILED; // hang
    }
    ...
    for (int i = 0; i < numInputs; i++) {
        ...
        memcpy(
            model.model_input[i]->data.int8, ((char *)KWS_power_example_input_tensors) + offset,
    ...
    while (1) {
        // Success!
        example_status = KWS_power_STATUS_SUCCESS;
    }
}
