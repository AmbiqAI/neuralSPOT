...
for TF in "R2.3.1" "0c46d6e" "fecdd5d"; do
    for AS in "R4.4.1" ; do
        ...
        make -f Makefile BOARD=apollo4l nestall > /dev/null > /dev/null && echo "Make Nestall AS_VERSION=$AS TF_VERSION=$TF success" || echo "Make Nestall AS_VERSION=$AS TF_VERSION=$TF failed"
        cd nest
        make -f Makefile BOARD=apollo4l > /dev/null > /dev/null && echo "Make nest AS_VERSION=$AS TF_VERSION=$TF success" || echo "Make nest AS_VERSION=$AS TF_VERSION=$TF failed"
        cd ..
