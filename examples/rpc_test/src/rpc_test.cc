...
status example_sendBlockToEVB(const dataBlock *block) {
    ...
    // ...
    // ns_lp_printf(".");
    for (uint32_t i = 0; i < block->buffer.dataLength; i++) {
        if (i % 256 != block->buffer.data[i]) {
            ns_lp_printf("Error at index %d, exp %d got %d\n", i, i % 256, block->buffer.data[i]);
        }
    }
    // ns_lp_printf(".");
...
status example_computeOnEVB(const dataBlock *in, dataBlock *res) {
    ...
    // Check values
    for (uint32_t i = 0; i < in->buffer.dataLength; i++) {
        if (i % 256 != in->buffer.data[i]) {
            ns_lp_printf("Error at index %d, exp %d got %d\n", i, i % 256, in->buffer.data[i]);
        }
    }

    // Multiply the incoming block * 2
    for (i = 0; i < len; i++) {
        resultBuffer[i] = (in->buffer.data[i]) % 256;
    }
