...
typedef struct {
    int (*spiRead)(const void *buf, uint32_t bufLen, uint64_t reg, uint32_t regLen, uint32_t csPin);
    int (*spiWrite)(
...
typedef struct {
    ...
    ArducamTransport *transport; /**< Camera transport interface*/
...
//**********************************************
CamStatus cameraSetAbsoluteExposure(ArducamCamera *camera, uint32_t val);
