/**
 * @file model_ambiqsuite_example.cc
...
#include "model_ambiqsuite_example_tensors.h"
#include "ns_model.h"
...
int main(void) {
    ...
    am_util_stdio_printf("Initializing model_ambiqsuite model...");
    int status = model_ambiqsuite_minimal_init(&model); // model init with minimal defaults

    if (status == model_ambiqsuite_STATUS_FAILURE) {
    ...
    for (int i = 0; i < numInputs; i++) {
        ...
        memcpy(
            model.model_input[i]->data.int8, ((char *)model_ambiqsuite_example_input_tensors) + offset,
    ...
    if (invoke_status != kTfLiteOk) {
        while (1)
            example_status = model_ambiqsuite_STATUS_FAILURE; // invoke failed, so hang
    }
    ...
    for (int i = 0; i < numOutputs; i++) {
        ...
        if (0 != memcmp(
                     ...
                     ((char *)model_ambiqsuite_example_output_tensors) + offset,
                     model.model_output[i]->bytes)) {
            // Miscompare!
            am_util_stdio_printf("Miscompare in output tensor %d\n", i);

            while (1)
                example_status = model_ambiqsuite_STATUS_INVALID_CONFIG; // miscompare, so hang
        }
    ...
    while (1) {
        // Success!
        example_status = model_ambiqsuite_STATUS_SUCCESS;
    }
}
