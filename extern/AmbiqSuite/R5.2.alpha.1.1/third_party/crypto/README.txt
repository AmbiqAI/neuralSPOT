...
using GCC and a bash or bash-like environment.
    ...
    Bare metal:
        1. From the third_party SDK folder, rename the crypto folder to crypto_sdk

        2. From the third_party SDK folder, dlone the public cryptocell-312 repository
           (see address above) to a directory parallel to this one named crypto

           For example (from the third_party directory):

           git clone https://github.com/ARM-software/cryptocell-312-runtime.git crypto

        4. Change directory to the third_party/crypto folder

        5. Take the ambiq-changes-r1p4.patch file from this directory, and apply
           it to the newly cloned crypto repository. This patch should be
           current as of version "update-cc110-bu-00000-r1p4".

           Example : patch -p1 < ../crypto_sdk/ambiq-changes-r1p4.patch

        6. Run "source set_env_for_gcc.sh cortex-%"; where % is either m4 or m55
           (Ambiq script to set environment variables for the next step.)

        7. ./prepare_mbedtls.sh clone

        8. Take the mbedtls.patch file from this directory, and apply
           it to the newly cloned mbedtls repository. This patch should be
           current as of version "2.16.2".

           Example: patch -p1 < ../crypto_sdk/mbedtls.patch

        9. ./prepare_mbedtls.sh lib

        10. make -C host/src ARM_CPU=$ARM_CPU TEE_OS=$TEE_OS
