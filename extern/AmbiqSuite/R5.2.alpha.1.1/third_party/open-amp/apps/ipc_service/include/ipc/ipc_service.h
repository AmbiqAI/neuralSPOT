...
extern "C"
{
    ...
    struct backend_config
    {
        ...
        size_t shm_size;
        /** 32-bit mailbox data for this instance to send buffer (generating kick interrupt to peer device) */
        uint32_t mbox_data_send;
        /** 32-bit mailbox data for this instance to receive buffer (generating kick interrupt from peer device) */
        uint32_t mbox_data_rcv;
        /**
         * @brief Send the mailbox magic number to nofity the peer that a new data has been written.
         *
         * @param data mailbox magic number.
         */
		void (*mbox_send)(uint32_t data);
        /**
         * @brief Initialize the IPC mailbox.
         */
		void (*mbox_init)(void);
        /** Buffer size of each packet */
     ...
     */
    int ipc_service_open_instance(struct ipc_device *instance);
     ...
     *  @retval other errno codes depending on the implementation of the backend.
     */
    int ipc_service_close_instance(const struct ipc_device *instance);
     ...
     *         endpoint's received cbk, this function should be called in application after getting notice
     *         from mailbox ISR.
     *
