...
#include "model_power_example_tensors.h"
#include "ns_model.h"
...
int main(void) {
    ...
    int status = model_power_minimal_init(&model); // model init with minimal defaults
    ns_interrupt_master_enable();
    if (status == model_power_STATUS_FAILURE) {
        while (1)
            ns_lp_printf("Model init failed.\n");
        example_status = model_power_STATUS_INIT_FAILED; // hang
    }
    ...
    for (int i = 0; i < numInputs; i++) {
        memcpy(
            model.model_input[i]->data.int8, ((char *)model_power_example_input_tensors) + offset,
    ...
    while (1) {
        // Success!
        example_status = model_power_STATUS_SUCCESS;
    }
}
