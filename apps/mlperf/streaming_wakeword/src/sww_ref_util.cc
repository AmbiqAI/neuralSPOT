...
int16_t g_i2s_buffer0[2048/sizeof(int16_t)];
int16_t g_i2s_buffer1[2048/sizeof(int16_t)];
...
// 	// g_i2s_buffer0 = (int16_t *)ns_malloc(g_i2s_chunk_size_bytes);
// 	// g_i2s_buffer1 = (int16_t *)ns_malloc(g_i2s_chunk_size_bytes);
// 	g_i2s_current_buff = g_i2s_buffer0;
// 	// g_wav_block_buff = (int16_t *)ns_malloc(SWW_WINLEN_SAMPLES * sizeof(int16_t));
// 	// g_model_input = (int8_t *)ns_malloc(SWW_MODEL_INPUT_SIZE * sizeof(int8_t));
	
// 	if (!g_i2s_buffer0 || !g_i2s_buffer1 || !g_wav_block_buff || !g_model_input){
// 		g_buffer_alloc_success = 0;
// 		th_printf("ERROR: Buffer allocation failed.  Many operationw will fail.\r\n");
// 	}
// 	else {
// 		g_buffer_alloc_success = 1;
// 	}
// 	if( !g_gp_buff_bytes) {
// 		th_printf("WARNING: general-purpose buffer allocation failed.  Wav and activation capture will fail.\r\n");
// 	}
// }
...
static void process_chunk_and_cont_capture(int16_t *idle_buffer)
{
    ...
    /*
     * No need to flip g_i2s_buff_sel or arm DMA again – the ISR already
     * called am_hal_i2s_interrupt_service(), which restarted the transfer,
     * and we’re looking at the buffer the DMA just finished.
     *
     * We *can* keep g_i2s_current_buff in sync for legacy code:
     */
    g_i2s_buff_sel     ^= 1;
    ...
    {
        /* Still room for more ‑ nothing else to do. */
    }
    else
    {
...
{
    // /* --- Scratch buffers & static work space --- */
    // static float32_t feature_buff[SWW_WINLEN_SAMPLES];
    // static float32_t dsp_buff[SWW_WINLEN_SAMPLES];
    // static int       num_calls = 0;

    // set_processing_pin_high();   // duty‑cycle profiling

    // /* Input‑scale factor from TFLM tensor metadata */
    // float32_t input_scale_factor =
    //     *(ai_input[0].meta_info->intq_info->info->scale);

    // /* ----------------------------------------------------------------
    //  * Move old samples in g_wav_block_buff left by WINSTRIDE,
    //  * then append WINSTRIDE new mono samples from idle_buffer (left channel).
    //  * ---------------------------------------------------------------- */
    // for (int i = SWW_WINSTRIDE_SAMPLES; i < SWW_WINLEN_SAMPLES; ++i)
    // {
    //     g_wav_block_buff[i - SWW_WINSTRIDE_SAMPLES] = g_wav_block_buff[i];
    ...
    // /* Slide window in g_model_input[] and append new 40‑bin vector */
    // memmove(g_model_input,
    //         g_model_input + NUM_MEL_FILTERS,
    //         (SWW_MODEL_INPUT_SIZE - NUM_MEL_FILTERS));

    // for (int i = 0; i < NUM_MEL_FILTERS; ++i)
    // {
    //     g_model_input[SWW_MODEL_INPUT_SIZE - NUM_MEL_FILTERS + i] =
    //         (int8_t)(feature_buff[i] / input_scale_factor - 128);
    // }

    // /* Copy to inference input tensor */
    // for (int i = 0; i < AI_SWW_MODEL_IN_1_SIZE; ++i)
    // {
    //     in_data[i] = (ai_i8)g_model_input[i];
    // }
    ...
    // /* Detection GPIO pulse */
    // if (out_data[0] > DETECT_THRESHOLD || g_first_frame)
    // {
    //     // am_hal_gpio_state_write(WW_DETECTED_Pin, AM_HAL_GPIO_OUTPUT_TOGGLE);
    //     ns_delay_us(1);
    //     g_first_frame = 0;
    // }

    // /* Log activations if buffer has room */
...
extern "C" void am_dspi2s0_isr(void)
{
    ...
    // 5. Dispatch exactly like the STM32 callback
    switch (g_i2s_state)
    {
        case FileCapture:
