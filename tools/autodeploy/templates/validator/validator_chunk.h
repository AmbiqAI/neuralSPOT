...
static inline void ns_chunk_advance(ns_chunk_t* c, uint32_t n){
  if (!c->active) return;
  c->progressed += n;
  if (c->progressed >= c->total) {
    c->progressed = c->total;
    c->active = false;
  }
}

static inline bool ns_chunk_done(const ns_chunk_t* c){
  return ns_chunk_remaining(c) == 0u;
}

static inline bool ns_chunk_is_last(const ns_chunk_t* c, uint32_t n){
  return (c->progressed + n) >= c->total;
}
