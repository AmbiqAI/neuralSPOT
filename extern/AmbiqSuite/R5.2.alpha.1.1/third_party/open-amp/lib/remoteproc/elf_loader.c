...
#include <string.h>
#include "../../../open-amp/libmetal/lib/alloc.h"
#include "../../../open-amp/libmetal/lib/log.h"
...
static void elf_parse_segment(void *elf_info, const void *elf_phdr,
                    ...
                    unsigned int *p_flag, unsigned int *p_align)
{
    if (elf_is_64(elf_info) == 0) {
        ...
        if (p_flag)
            *p_flag = (unsigned int)phdr->p_flags;
        if (p_align)
            *p_align = (unsigned int)phdr->p_align;
    } else {
...
static const void *elf_next_load_segment(void *elf_info, int *nseg,
				   ...
				   size_t *nmsize, unsigned int *nflag)
{
	...
	while (p_type != PT_LOAD) {
		...
		elf_parse_segment(elf_info, phdr, &p_type, noffset,
				  da, NULL, nfsize, nmsize, nflag, NULL);
		*nseg = *nseg + 1;
	}
	return phdr;
}
                ...
                size_t *offset, size_t *size, unsigned int *flags)
{
    ...
    shdr = elf_get_section_from_name(elf_info, name);
    if (!shdr) {
        metal_assert(size);
        *size = 0;
        return -RPROC_EINVAL;
    }
    elf_parse_section(elf_info, shdr, NULL, flags,
                da, offset, size,
                NULL, NULL, NULL, NULL);
    return 0;
}
...
{
    size_t header_size, phdrs_offset, shdrs_offset, phent_size,
            ph_number, shent_size, sh_number, infosize;
    int shstr_index;

    header_size = elf_ehdr_size(img_data);
    ...
    memcpy(*img_info, img_data, header_size);
    phdrs_offset = elf_phoff(*img_info);
    shdrs_offset = elf_shoff(*img_info);
    phent_size   = elf_phentsize(*img_info);
    ph_number    = elf_phnum(*img_info);
    shent_size   = elf_shentsize(*img_info);
    sh_number    = elf_shnum(*img_info);
    shstr_index  = elf_shstrndx(*img_info);
    am_util_stdio_printf("================================================================\n");
    am_util_stdio_printf("ELF Header:\n");
    am_util_stdio_printf("Start of program headers:          \t%ld (bytes into file)\n",
                    phdrs_offset);
    am_util_stdio_printf("Start of section headers:          \t%ld (bytes into file)\n",
                    shdrs_offset);
    am_util_stdio_printf("Size of this header:               \t%ld (bytes)\n",
                    header_size);
    am_util_stdio_printf("Size of program headers:           \t%ld (bytes)\n",
                    phent_size);
    am_util_stdio_printf("Number of program headers:         \t%ld\n",
                    ph_number);
    am_util_stdio_printf("Size of section headers:           \t%ld (bytes)\n",
                    shent_size);
    am_util_stdio_printf("Number of section headers:         \t%ld\n",
                    sh_number);
    am_util_stdio_printf("Section header string table index: \t%ld\n",
                shstr_index);


    /* Get ELF program headers */
    size_t phdrs_size, ph_offset, ph_fsize, ph_msize;
    metal_phys_addr_t ph_vaddr, ph_paddr;
    int ph_index;
    void **phdrs;
    const void *img_phdrs;
    const void *phdr = PT_NULL;
    unsigned int ph_type = PT_NULL;
    unsigned int ph_flag, ph_align;
    ...
    *phdrs = metal_allocate_memory(phdrs_size);
    ...
    memcpy(*phdrs, img_phdrs, phdrs_size);
    am_util_stdio_printf("\n================================================================\n");
    am_util_stdio_printf("program header\n");
    for (ph_index = 0; ph_index < ph_number; ph_index++) {
        phdr = elf_get_segment_from_index(*img_info, ph_index);
        if (!phdr)
            return -RPROC_EINVAL;
        elf_parse_segment(*img_info, phdr, &ph_type, &ph_offset,
                    &ph_vaddr, &ph_paddr, &ph_fsize, &ph_msize, &ph_flag, &ph_align);
        am_util_stdio_printf("%d\t\t    ", ph_type);
        am_util_stdio_printf("\t\toffset 0x%06x", ph_offset);
        am_util_stdio_printf("\t\tVirtAddr 0x%08x", ph_vaddr);
        am_util_stdio_printf("\t\tPhysAddr 0x%08x", ph_paddr);
        am_util_stdio_printf("\t\tFileSiz 0x%05x", ph_fsize);
        am_util_stdio_printf("\t\tMemSiz 0x%05x", ph_msize);
        am_util_stdio_printf("\t\tFlag %c%c%c",
                        (ph_flag & PF_R ? 'R' : ' '),
                        (ph_flag & PF_W ? 'W' : ' '),
                        (ph_flag & PF_X ? 'E' : ' '));
        am_util_stdio_printf("\t\tAlign 0x%x\n", ph_align);
    }
    am_util_stdio_printf("\n================================================================\n");
    ...
    *shdrs = metal_allocate_memory(shdrs_size);
    ...
    size_t shstrtab_size;
    size_t shstrtab_offset;
    // metal_phys_addr_t shstrtab_addr;
    // unsigned int shstrtab_addralign;
    void *shdr;
    void **shstrtab;

    shdr = elf_get_section_from_index(*img_info, shstr_index);
    if (!shdr)
        return -RPROC_EINVAL;
    elf_parse_section(*img_info, shdr, NULL, NULL,
            NULL, &shstrtab_offset,
...
int elf_load_ph_header(void *img_data, void **img_info, int *ph_number)
{
    ...
    *ph_number = elf_phnum(*img_info);
    ...
    /* Get ELF program headers */
    size_t phdrs_size, phdrs_offset;
    void **phdrs;
    const void *img_phdrs;
    ...
    *phdrs = metal_allocate_memory(phdrs_size);
    if (!*phdrs)
        return -RPROC_ENOMEM;
    memcpy(*phdrs, img_phdrs, phdrs_size);
    am_util_stdio_printf("Loading ELF program header.\r\n");
    return RPROC_EBASE;
}
                    ...
                    size_t *ph_msize, unsigned int *ph_flag, unsigned int *ph_align)
{
    ...
    if (*img_info == NULL)
        return -RPROC_EINVAL;
    phdr = elf_get_segment_from_index(*img_info, index);
    if (!phdr)
        return -RPROC_EINVAL;
    elf_parse_segment(*img_info, phdr, ph_type, ph_offset,
                ph_vaddr, ph_paddr, ph_fsize, ph_msize, ph_flag, ph_align);
...
{
    size_t header_size, shdrs_offset, infosize;
    int shstr_index;
    ...
    *shstrtab = metal_allocate_memory(shstrtab_size);
    ...
    /* parsing section by section name */
    shdr = elf_get_section_from_name(*img_info, s_name);
    if (!shdr)
        return -RPROC_EINVAL;
    elf_parse_section(*img_info, shdr, s_type, s_flags,
                s_addr, s_offset, s_size,
                NULL, NULL, s_align, NULL);
