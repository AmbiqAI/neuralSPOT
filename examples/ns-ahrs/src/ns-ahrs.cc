...
// #endif
    ...
    while (1) {
        ...
        // nxp_update(&filter, finalGyro[0], finalGyro[1], finalGyro[2], finalAccel[0], finalAccel[1], finalAccel[2], finalAccel[0], finalAccel[1], finalAccel[2]);
...
// int
// main(void) {
//     ns_perf_counters_t pp, pp_start, pp_end;
//     uint16_t sample = 0;
//     ns_core_config_t ns_core_cfg = {.api = &ns_core_V1_0_0};
//     // myState_e state = WAITING_TO_RECORD;
...
//     ns_capture_perf_profiler(&pp_start);
//     nxp_begin(&filter, sampleFrequency);
//     ns_stop_perf_profiler();
//     ns_capture_perf_profiler(&pp_end);
//     ns_delta_perf(&pp_start, &pp_end, &pp);
...
//             gyroVals1[axis] = mpu6050_gyro_to_deg_per_sec(gyroVals[axis], GYRO_FS_500DPS);

//             accelVals1[axis] = mpu6050_accel_to_gravity(accelVals[axis], ACCEL_FS_4G);
//         }
//         ns_lp_printf("accel values: %f, %f, %f\n", accelVals1[0], accelVals1[1], accelVals1[2]);
//         ns_lp_printf("gyro values: %f, %f, %f\n", gyroVals1[0], gyroVals1[1], gyroVals1[2]);
//         nxp_update(&filter, gyroVals1[0], gyroVals1[1], gyroVals1[2], accelVals1[0], accelVals1[1], accelVals1[2], 1, 1, 1);
//         // print the heading, pitch and roll
//         roll = nxp_getRoll(&filter);
//         pitch = nxp_getPitch(&filter);
//         heading = nxp_getYaw(&filter);
