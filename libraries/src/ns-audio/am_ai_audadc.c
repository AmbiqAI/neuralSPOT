...
audio_callback(void)
{
    ...
    if (g_audioRecording) {
        ...
        if (g_audioReady) {
            am_util_stdio_printf("Warning - audio buffer wasnt consumed in time\n");
        }
...
am_audadc0_isr(void)
{
    ...
    if (ui32IntMask & AM_HAL_AUDADC_INT_FIFOOVR1)
    {
        if ( AUDADCn(0)->DMASTAT_b.DMACPL )
        {
            ...
            am_ai_audio_config.callback(&am_ai_audio_config, 0);
        }
    }
...
{    
    ...
    //
    g_sAUDADCDMAConfig.ui32SampleCount = am_ai_audio_config.numSamples;
    ...
    //
    audadc_config();
    am_ai_audio_config.audioSystemHandle = g_AUDADCHandle; // wait for it to have real value
