...
def send_data(ser: serial.Serial, debug_level: int, delay_ms: int) -> None:
    ...
    while not stop_event.is_set():
        ...
        block_size: int = RX_DMA_SIZE if mode_rxDma else random.randint(1, MAX_BLOCK_SIZE)
...
def main() -> None:
    ...
    parser.add_argument('-p', '--port', type=str, default="COM3", help="Serial port (e.g., COM4 or /dev/ttyUSB0)")
    ...
    parser.add_argument('-d', '--debug', type=int, choices=[0, 1, 2], default=2, help="Debug level: 0 (no output), 1 (sent data), 2 (sent and received data)")
    parser.add_argument('--delay', type=int, default=400, help="Delay between blocks in milliseconds (default: 500)")
    ...
    stop_bits : int = serial.STOPBITS_TWO if baud_rate >= 1_500_000 else serial.STOPBITS_ONE
