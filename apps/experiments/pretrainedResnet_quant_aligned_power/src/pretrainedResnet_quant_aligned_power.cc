/**
 * @file pretrainedResnet_quant_aligned_power_example.cc
...
// #endif
#if 0 == 1
#include "pretrainedResnet_quant_aligned_model.h"
#include "pretrainedResnet_quant_aligned_platform.h"         // platform macros
#include "pretrainedResnet_quant_aligned_tensors.h"          // tensor sizes
#include "pretrainedResnet_quant_aligned_context.h"          // model context
#include "pretrainedResnet_quant_aligned_power_aot_api.h"
#include "pretrainedResnet_quant_aligned_power_aot_example_tensors.h"
...
#include "pretrainedResnet_quant_aligned_power_example_tensors.h"
#include "ns_model.h"
#endif
...
static void aot_callback(int32_t op,
                         pretrainedResnet_quant_aligned_operator_state_t state,
                         int32_t status,
                         void *user_data)
{
    ...
    if (state == pretrainedResnet_quant_aligned_op_state_run_started) {
        ns_pmu_accm_op_begin(aot_matrix, op);
    }

    if (state == pretrainedResnet_quant_aligned_op_state_run_finished) {
        ns_pmu_accm_op_end(aot_matrix, op);

    }
}

static pretrainedResnet_quant_aligned_model_context_t model = {
    // .input_data = {pretrainedResnet_quant_aligned_power_example_input_tensors},
    // .output_data = {pretrainedResnet_quant_aligned_power_output_tensors},
    #if 1 == 0
...
int main(void) {
    ...
    // memcpy(model.outputs[0].data, pretrainedResnet_quant_aligned_outputs, sizeof(pretrainedResnet_quant_aligned_outputs));
    // memcpy(model.input_len, pretrainedResnet_quant_aligned_inputs_len, sizeof(pretrainedResnet_quant_aligned_inputs_0_size));
    // memcpy(model.output_len, pretrainedResnet_quant_aligned_outputs_len, sizeof(pretrainedResnet_quant_aligned_outputs_0_size));
    ns_lp_printf("Initializing AOT model...\n");
    int status = pretrainedResnet_quant_aligned_model_init(&model); // model init with minimal defaults
    memcpy(model.inputs[0].data, pretrainedResnet_quant_aligned_power_example_input_tensors, sizeof(pretrainedResnet_quant_aligned_power_example_input_tensors));
    ...
    // }
#else
    int status = pretrainedResnet_quant_aligned_power_minimal_init(&model); // model init with minimal defaults
#endif
    if (status == pretrainedResnet_quant_aligned_power_STATUS_FAILURE) {
        ns_lp_printf("TFLM Model init failed.\n");
        while (1);
        example_status = pretrainedResnet_quant_aligned_power_STATUS_INIT_FAILED; // hang
    }
    ...
    for (int i = 0; i < numInputs; i++) {
        ...
        memcpy(
            model.model_input[i]->data.int8, ((char *)pretrainedResnet_quant_aligned_power_example_input_tensors) + offset,
    ...
    while (1) {
        ...
        case RUNNING:
            ...
            // for (int i = 0; i < 200; i++) {
                ...
                pretrainedResnet_quant_aligned_model_run(&model);
    ...
    while (1) {
        // Success!
        example_status = pretrainedResnet_quant_aligned_power_STATUS_SUCCESS;
    }
}
