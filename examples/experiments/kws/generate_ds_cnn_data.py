...
def main():
    ...
    # For conv1, we use the same tensor as its weights.
    # For depthwise conv, we match on "depthwise_conv2d".
    # For pointwise conv, we use the block1 pointwise weight tensor pattern.
    # For fc, we use the "dense/MatMul" pattern.
    quant_mapping = {
        "conv1_multiplier": "functional_1/conv2d/Conv2D",
        "conv1_shift":      "functional_1/conv2d/Conv2D",
        "dw_conv_multiplier": "depthwise_conv2d",  # matches any tensor with "depthwise_conv2d" in its name.
