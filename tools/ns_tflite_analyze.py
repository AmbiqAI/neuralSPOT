...
#
# --- New helpers: dtype & activations & elementwise hints --------------------
#
_TTYPE_TO_NAME = {
    0: "float32",   # FLOAT32
    1: "float16",   # FLOAT16
    ...
    6: "bool",      # BOOL
    7: "int16",     # INT16
    8: "complex64", # COMPLEX64
    9: "int8",      # INT8
    10:"float64",   # FLOAT64
    11:"complex128",# COMPLEX128
    12:"uint64",    # UINT64
    13:"resource",  # RESOURCE
    14:"variant",   # VARIANT
    15:"uint32",    # UINT32
    16:"uint16",    # UINT16
    17:"int4",      # INT4  (recent schema)
    18:"bfloat16",  # BF16
    19:"float8_e5m2",
    20:"float8_e4m3fn",
}
...
_TTYPE_TO_BITS = {
    "float32": 32, "float16": 16, "int32": 32, "uint8": 8,  "int64": 64,
    "bool": 1,     "int16": 16,   "int8": 8,   "float64": 64, "uint64": 64,
    "uint32": 32,  "uint16": 16,  "int4": 4,   "bfloat16": 16,
    "float8_e5m2": 8, "float8_e4m3fn": 8,
}

def _dtype_name(ttype_value):
    if isinstance(ttype_value, str):
        # Some parsers already use names
        return ttype_value.lower()
    return _TTYPE_TO_NAME.get(int(ttype_value), f"t{ttype_value}")

def _dtype_bits(dtype_name: str) -> int:
    return _TTYPE_TO_BITS.get(dtype_name, 8)
...
def _get_fused_activation(op_builtin_opts) -> str:
    if not isinstance(op_builtin_opts, dict):
        return "NONE"
    val = op_builtin_opts.get("fused_activation_function", 0)
    if isinstance(val, str):
        return val.upper()
    return _FUSED_ACT_MAP.get(int(val), "NONE")

def _lanes_for_bits(bits: int) -> int:
    b = 8 if bits == 4 else bits   # treat int4 widened later if needed
    if b <= 0: return 0
    return 128 // b

_ELEMENTWISE_OPS = {
    "ADD","SUB","MUL","DIV","MAXIMUM","MINIMUM",
    "ABS","NEG","SQRT","RSQRT","EXP","LOG","SIN","COS",
    "TANH","LOGISTIC","RELU","RELU6","RELU_N1_TO_1","LEAKY_RELU",
    "HARD_SWISH","SQUARE","POWER","FLOOR","CEIL","ROUND",
    "CLAMP","PAD",  # PAD still moves bytes – keep hints for now
}
...
def analyze_tflite_file(tflite_file, model_name="", verbose=True):  # pylint: disable=invalid-name
    ...
    # New: dtype & activation & elementwise-hints (per subgraph, per op)
    overall_input_dtype_list = []
    overall_output_dtype_list = []
    overall_weight_dtype_list = []
    overall_fused_activation_list = []
    overall_elem_bits_list = []
    overall_elem_vec_chunks_list = []
    overall_elem_predicated_list = []
    ...
    for subgraph_idx, g in enumerate(data["subgraphs"]):
        ...
        overall_input_dtype_list.append([])
        overall_output_dtype_list.append([])
        overall_weight_dtype_list.append([])
        overall_fused_activation_list.append([])
        overall_elem_bits_list.append([])
        overall_elem_vec_chunks_list.append([])
        overall_elem_predicated_list.append([])
        ...
        if g["operators"]:
            for idx, tensor in enumerate(g["operators"]):
                ...
                inputs_idx = tensor.get("inputs", []) or []
                outputs_idx = tensor.get("outputs", []) or []
                ...
                def _tensor_dtype_name(ti):
                    try:
                        ...
                        return "unknown"
                in_dtype  = _tensor_dtype_name(inputs_idx[0]) if len(inputs_idx) >= 1 else "unknown"
                out_dtype = _tensor_dtype_name(outputs_idx[0]) if len(outputs_idx) >= 1 else "unknown"
                # weights at input[1] for conv/fc/dw
                if opcodeName in ["CONV_2D","DEPTHWISE_CONV_2D","FULLY_CONNECTED"] and len(inputs_idx) >= 2:
                ...
                # --- New: fused activation from builtin options ---
                fused_act = _get_fused_activation(tensor.get("builtin_options", {}))
                overall_fused_activation_list[subgraph_idx].append(fused_act)

                # --- New: elementwise vectorization hints ---
                # Treat "elementwise" as either known op OR generic non-MAC ops with output elements
                is_elementwise = (opcodeName in _ELEMENTWISE_OPS) or (macs == 0 and output_magnitude > 0)
                if is_elementwise:
                    bits  = _dtype_bits(out_dtype)
                    lanes = _lanes_for_bits(bits)
                    vec_chunks = math.ceil(output_magnitude / lanes) if lanes else 0
                    predicated = 1 if (lanes and (output_magnitude % lanes != 0)) else 0
                else:
    ...
    code_fused_act = ""
    code_elem_bits = ""
    code_elem_vec_chunks = ""
    code_elem_predicated = ""
    ...
    for g in range(len(overall_output_shapes)-1, -1, -1):
        ...
        code_in_dtype   = code_in_dtype   + repr(overall_input_dtype_list[g]) + ", "
        code_out_dtype  = code_out_dtype  + repr(overall_output_dtype_list[g]) + ", "
        code_w_dtype    = code_w_dtype    + repr(overall_weight_dtype_list[g]) + ", "
        code_fused_act  = code_fused_act  + repr(overall_fused_activation_list[g]) + ", "
    ...
    code_in_dtype   = code_in_dtype.replace("[", "").replace("]", "").replace("'", '"')
    code_out_dtype  = code_out_dtype.replace("[", "").replace("]", "").replace("'", '"')
    code_w_dtype    = code_w_dtype.replace("[", "").replace("]", "").replace("'", '"')
    code_fused_act  = code_fused_act.replace("[", "").replace("]", "").replace("'", '"')
    code_elem_bits        = code_elem_bits.replace("[", "").replace("]", "")
    code_elem_vec_chunks  = code_elem_vec_chunks.replace("[", "").replace("]", "")
    code_elem_predicated  = code_elem_predicated.replace("[", "").replace("]", "")
    ...
    code_in_dtype   = f"const char* {model_name}_in_dtype[] = {{" + code_in_dtype + "};\n"
    code_out_dtype  = f"const char* {model_name}_out_dtype[] = {{" + code_out_dtype + "};\n"
    code_w_dtype    = f"const char* {model_name}_w_dtype[] = {{" + code_w_dtype + "};\n"
    code_fused_act  = f"const char* {model_name}_fused_act[] = {{" + code_fused_act + "};\n"
    code_elem_bits        = f"const uint32_t {model_name}_elem_bits[] = {{" + code_elem_bits + "};\n"
    code_elem_vec_chunks  = f"const uint32_t {model_name}_elem_vec_chunks[] = {{" + code_elem_vec_chunks + "};\n"
    code_elem_predicated  = f"const uint32_t {model_name}_elem_predicated[] = {{" + code_elem_predicated + "};\n"
    ...
    code = code_mac_strings + code_output_shapes + code_output_magnitude + code_stride_h + code_stride_w + code_dilation_h + code_dilation_w + code_mac_filter + code_write_estimate + code_read_estimate + code_in_dtype + code_out_dtype + code_w_dtype + code_fused_act + code_elem_bits + code_elem_vec_chunks + code_elem_predicated
    # code = (
    #     code_mac_strings + code_output_shapes + code_output_magnitude +
    #     code_stride_h + code_stride_w + code_dilation_h + code_dilation_w +
    #     code_mac_filter + code_write_estimate + code_read_estimate +
    #     code_in_dtype + code_out_dtype + code_w_dtype + code_fused_act +
    #     code_elem_bits + code_elem_vec_chunks + code_elem_predicated
    # )
...
def main(argv):
    try:
        ...
        (
            code, ops, macs, mac_strings, opset, slen,
            overall_output_shapes, overall_dilation_h, overall_dilation_w,
            overall_stride_h, overall_stride_w, overall_mac_filter_list,
            overall_read_estimate_list, overall_write_estimate_list,
            output_magnitude_list, input_magnitude_list,
            in_dtypes, out_dtypes, w_dtypes, fused_acts,
            elem_bits, elem_vec_chunks, elem_predicated
        ) = analyze_tflite_file(tflite_input, verbose=True)
        ...
        headers = [
            "Subgraph","Index","Ops",
            "Estimated_MACs","MAC_Eq",
            "Input_DType","Weight_DType","Output_DType","FusedActivation",
            "Output_Shape","Dilation_H","Dilation_W","Stride_H","Stride_W",
            "Filter_Shape","Estimated_Reads","Estimated_Writes",
            "Input_Magnitude","Output_Magnitude",
            "ElemBits","ElemVecChunks","ElemPredicatedInstrs"
        ]
        table = [headers]
        for subgraph in range(slen):
            for op in range(len(ops[subgraph])):
                ...
                row = [
                    subgraph, op, ops[subgraph][op],
                    macs[subgraph][op], mac_strings[subgraph][op],
                    in_dtypes[subgraph][op], w_dtypes[subgraph][op], out_dtypes[subgraph][op], fused_acts[subgraph][op],
                    overall_output_shapes[subgraph][op],
                    overall_dilation_h[subgraph][op], overall_dilation_w[subgraph][op],
                    overall_stride_h[subgraph][op], overall_stride_w[subgraph][op],
                    overall_mac_filter_list[subgraph][op],
                    overall_read_estimate_list[subgraph][op], overall_write_estimate_list[subgraph][op],
                    input_magnitude_list[subgraph][op], output_magnitude_list[subgraph][op],
