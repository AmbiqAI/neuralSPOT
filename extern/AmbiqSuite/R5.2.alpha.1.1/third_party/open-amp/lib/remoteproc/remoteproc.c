...
#include "../../../open-amp/libmetal/lib/utilities.h"
		    ...
		    void **img_info)
{
	...
	while (1) {
		...
		ret = loader->load_data(rproc, img_data, offset, len,
					...
					&noffset, &nlen, &padding, &nmemsize, &flag);
		if (ret < 0) {
			metal_log(METAL_LOG_ERROR,
		...
		if (da != RPROC_LOAD_ANYADDR) {
			...
			pa = METAL_BAD_PHYS;
			(void)remoteproc_mmap(rproc, &pa, &da, nmemsize, flag,
					      NULL);
			    ...
			    size_t *nmlen, unsigned char *padding)
{
	...
	*nlen = 0;
	...
	if ((last_load_state & RPROC_LOADER_READY_TO_LOAD) != 0 ||
		...
		ret = loader->load_data(rproc, img_data, offset, len,
					&limg_info, last_load_state, &da,
					noffset, nlen, padding, nmlen, NULL);
		metal_log(METAL_LOG_DEBUG,
            ...
            void **img_info, const char *sect_name)
{
	...
	/* load codec data */
    metal_log(METAL_LOG_DEBUG, "%s: load codec data\r\n", __func__);
    offset = 0;
    len = 0;
    unsigned int  flag;
    size_t nmemsize;
    ...
    ret = loader->load_section(limg_info, sect_name, &da,
                    &noffset, &nlen, &flag);

    if (ret < 0) {
        metal_log(METAL_LOG_ERROR,
                "load codec failed,0x%lx,%d\r\n",
                noffset, nlen);
        goto error2;
    }
    metal_log(METAL_LOG_DEBUG,
            "load codec: da 0x%lx, offset 0x%lx, len = 0x%lx, memsize = 0x%lx, state 0x%x\r\n",
            da, noffset, nlen, ret);
    ...
    metal_log(METAL_LOG_DEBUG, "%s: successfully load codec\r\n",
            __func__);
    rproc->state = RPROC_READY;
