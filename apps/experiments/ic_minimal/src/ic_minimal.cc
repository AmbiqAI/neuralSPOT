...
#if 0 == 1
#include "IC_model.h"
#include "IC_platform.h"         // platform macros
#include "IC_tensors.h"          // tensor sizes
#include "IC_context.h"          // model context
#include "IC_power_aot_api.h"
#include "IC_power_aot_example_tensors.h"
...
#include "IC_power_example_tensors.h"
#include "ns_model.h"
#endif
...
static uint32_t aot_matrix_store[16 * NS_PMU_MAP_SIZE];
                         ...
                         void *user_data)
{
    ...
    if (state == IC_op_state_run_started) {
        ns_pmu_accm_op_begin(aot_matrix, op);
    }

    if (state == IC_op_state_run_finished) {
        ns_pmu_accm_op_end(aot_matrix, op);

    }
}

static IC_model_context_t model = {
    // .input_data = {IC_power_example_input_tensors},
    // .output_data = {IC_power_output_tensors},
    #if 1 == 0
...
int main(void) {
    ...
    // memcpy(model.outputs[0].data, IC_outputs, sizeof(IC_outputs));
    // memcpy(model.input_len, IC_inputs_len, sizeof(IC_inputs_0_size));
    // memcpy(model.output_len, IC_outputs_len, sizeof(IC_outputs_0_size));
    ns_lp_printf("Initializing AOT model...\n");
    int status = IC_model_init(&model); // model init with minimal defaults
    memcpy(model.inputs[0].data, IC_power_example_input_tensors, sizeof(IC_power_example_input_tensors));
    ...
    // }
#else
    int status = IC_power_minimal_init(&model); // model init with minimal defaults
#endif
    if (status == IC_power_STATUS_FAILURE) {
        ns_lp_printf("TFLM Model init failed.\n");
        while (1);
        example_status = IC_power_STATUS_INIT_FAILED; // hang
    }
    ...
    for (int i = 0; i < numInputs; i++) {
        ...
        memcpy(
            model.model_input[i]->data.int8, ((char *)IC_power_example_input_tensors) + offset,
    ...
    while (1) {
        ...
        case SIGNAL_END_TO_JS:
            ...
            aot_matrix = ns_pmu_accm_create(16, NS_PMU_MAP_SIZE, aot_matrix_store);
            ...
            ns_pmu_accmprint_matrix(aot_matrix, m, 16, NS_PMU_MAP_SIZE);
            ns_lp_printf("AOT matrix print complete.\n");
            // dump_csv(m, 16, NS_PMU_MAP_SIZE);
    ...
    while (1) {
        // Success!
        example_status = IC_power_STATUS_SUCCESS;
    }
}
