...
rm "build/examples/rpc_client/rpc_client.axf"           > /dev/null || true
rm "build/examples/rpc_server/rpc_server.axf"           > /dev/null || true
...
rm "build/examples/rpc_client/rpc_client.bin"           > /dev/null || true
rm "build/examples/rpc_server/rpc_server.bin"           > /dev/null || true
...
# cd ..
test -f "build/examples/rpc_client/rpc_client.bin"           || { echo "FAIL rpc client doesn't exit"; exit 1; }
test -f "build/examples/rpc_server/rpc_server.bin"           || { echo "FAIL rpc server doesn't exit"; exit 1; }
