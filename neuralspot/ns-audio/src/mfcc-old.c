...
arm_rfft_fast_instance_f32 g_mfccRfftold;
...
static void
create_dct_matrixold(int32_t input_length, int32_t coefficient_count) {
    int32_t k, n;
    float normalizer;
...
ns_mfcc_init_old(void) {
    ...
    create_mel_fbankold();
    create_dct_matrixold(MFCC_NUM_FBANK_BINS, MFCC_NUM_MFCC_FEATURES);

    arm_rfft_fast_init_f32(&g_mfccRfftold, MFCC_FRAME_LEN_POW2);
}
...
ns_mfcc_compute_old(const int16_t *audio_data, float *mfcc_out) {
    ...
    // Compute FFT
    arm_rfft_fast_f32(&g_mfccRfftold, g_mfccFrame, g_mfccBuffer, 0);
