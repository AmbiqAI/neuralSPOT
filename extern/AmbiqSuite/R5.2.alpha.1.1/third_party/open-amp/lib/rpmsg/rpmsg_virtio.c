...
/* Not with VQ_RX_EMPTY_NOTIFY */
#define VQ_RX_EMPTY_NOTIFY                      0
...
static bool rpmsg_virtio_buf_held_dec_test(struct rpmsg_hdr *rp_hdr)
{
	...
	if (RPMSG_BUF_HELD_COUNTER(rp_hdr) <= 0) {
		/* Fix the "ISO C99 requires at least one argument for the '...' in a
		 * variadic macro warning.
		 */
		metal_err("unexpected buffer held counter %d\r\n", RPMSG_BUF_HELD_COUNTER(rp_hdr));
		return false;
	}
