...
create_mel_fbank() {
    ...
    float mel_freq_delta =
        (mel_high_freq - mel_low_freq) / (NUM_FBANK_BINS + 1);
...
ns_mfcc_init(void) {
    ...
    for (i = 0; i < FRAME_LEN; i++) {
        g_mfccWindowFunction[i] =
            0.5 - 0.5 * cos(M_2PI * ((float)i) / (FRAME_LEN));
    }
...
ns_mfcc_compute(const int16_t *audio_data, float *mfcc_out) {
    ...
    memset(&g_mfccFrame[FRAME_LEN], 0,
           sizeof(float) * (FRAME_LEN_POW2 - FRAME_LEN));
