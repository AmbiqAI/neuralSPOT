...
static int8_t get_high_bits(int16_t regReading) {
    return (regReading & 0xFF00) >> 8;
}

static int8_t get_low_bits(int16_t regReading) {
...
uint32_t mpu6050_set_accel_offset(ns_i2c_config_t *cfg, uint32_t devAddr, uint8_t axis, int offset) {
    return (
        ns_i2c_write_reg(cfg, devAddr, (1 << axis) + MPU6050_RA_XA_OFFS_H, get_high_bits(offset), 0xFF) ||
...
uint32_t mpu6050_calibrate(ns_i2c_config_t *cfg, uint32_t devAddr) {
    ...
    while (1) {
        ...
        if (abs(meanAX) <= accelDeadzone) ready++;
