...
extern void sendAck(uint8_t chunk_id);
...
// Implementation of sendAck
void sendAck(uint8_t chunk_id) {
    // Create header
    usb_message_header_t header;
    header.crc32 = 0;  // No payload, so CRC is 0
    header.command = web::usb::Command_CHECK_HEARTBEAT;  // Use heartbeat as ack
    header.chunk_id = chunk_id;
    header.total_chunks = 1;
    
    // Send just the header
    webusb_send_data((uint8_t*)&header, sizeof(header));
}

// Modify msgReceived function to handle commands
void msgReceived(const uint8_t *buffer, uint32_t length, void *args) {
    ...
    uint32_t payload_length = length - sizeof(usb_message_header_t);
    
    // Verify CRC
    uint32_t computed_crc = CalcCrc32(0xFFFFFFFF, payload_length, (uint8_t*)payload);
    if (computed_crc != header->crc32) {
        am_util_stdio_printf("CRC mismatch: computed=0x%08x, received=0x%08x\n", 
    ...
    if (!flatbuffers::BufferHasIdentifier(payload, "USB1")) {
        am_util_stdio_printf("Invalid FlatBuffer identifier\n");
        return;
    }
    
    const web::usb::UsbPacket* packet = flatbuffers::GetRoot<web::usb::UsbPacket>(payload);
    if (!packet->Verify(verifier)) {
        am_util_stdio_printf("Invalid FlatBuffer\n");
        return;
    }
    
    // Handle different commands
    switch (header->command) {
        case web::usb::Command_SEND_DATA:
            // Handle different packet types
            switch (packet->type()) {
                case web::usb::UsbDataType_MODEL_DATA:
                    if (handle_model_chunk(packet->data()->data(), 
                                         packet->data()->size(),
                                         header->chunk_id,  // Use chunk_id as sequence
                                         header->total_chunks)) {
                        sendAck(header->chunk_id);
                    }
                    break;
                    
                case web::usb::UsbDataType_NONE:
                case web::usb::UsbDataType_SET_SE_MODE:
                case web::usb::UsbDataType_SE_LATENCY:
                case web::usb::UsbDataType_OPUS_LATENCY:
                case web::usb::UsbDataType_AUDIO_DATA:
                    // These types are handled by the sender, not the receiver
                    break;
                    
                default:
                    am_util_stdio_printf("Unknown packet type: %d\n", packet->type());
                    break;
            }
            break;
            
        case web::usb::Command_CHECK_MAILBOX:
        ...
        case web::usb::Command_CHECK_HEARTBEAT:
            // Respond to heartbeat
            sendAck(header->chunk_id);
            break;
            
        default:
            am_util_stdio_printf("Unknown command: %d\n", header->command);
            break;
    }
}
...
void send_chunked_data(web::usb::UsbDataType type, const uint8_t* data_ptr, size_t data_len) {
    ...
    for (size_t chunk = 0; chunk < num_chunks; chunk++) {
        ...
        auto data_vector = builder.CreateVector(
            data_ptr + (chunk * max_chunk_size), 
            chunk_size
        );
        
        auto packet = web::usb::CreateUsbPacket(
            builder,
            (web::usb::Platform)data.platform,  // Platform first
            0,  // sequence
            type,  // UsbDataType
            data_vector
        );
        ...
        usb_message_header_t header;
        header.crc32 = CalcCrc32(0xFFFFFFFF, len, buf);
        header.command = web::usb::Command_SEND_DATA;  // Use proper command enum
...
void set_se_mode(bool enable) {
    ...
    send_chunked_data(web::usb::UsbDataType_SET_SE_MODE, &data, 1);
}

void send_se_latency(uint32_t latency) {
    // ns_lp_printf("SE Latency %d\n", latency);
    send_chunked_data(web::usb::UsbDataType_SE_LATENCY, (uint8_t*)&latency, sizeof(latency));
}

void send_opus_latency(uint32_t latency) {
    // ns_lp_printf("Opus Latency %d\n", latency);
    send_chunked_data(web::usb::UsbDataType_OPUS_LATENCY, (uint8_t*)&latency, sizeof(latency));
}

void send_audio_data(unsigned char *buffer) {
    // ns_lp_printf("Sending %d bytes\n", sizeof(encodedDataBuffer));
    send_chunked_data(web::usb::UsbDataType_AUDIO_DATA, buffer, sizeof(encodedDataBuffer));
    ns_delay_us(600);
}
...
static model_upload_state_t* model_state = NULL;
...
int main(void) {
    ...
    model_upload_init();
    model_state = get_model_upload_state();
