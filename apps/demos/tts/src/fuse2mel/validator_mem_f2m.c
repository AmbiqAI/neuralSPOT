...
#include "mut_model_metadata_f2m.h"   // buffer sizes & locations

// Generated model data when not streaming via PSRAM
#if (TFLM_MODEL_LOCATION_F2M != NS_AD_PSRAM) && (NS_AD_AOT == 0)
#include "mut_model_data_f2m.h"
#endif
...
#if (TFLM_ARENA_LOCATION_F2M == NS_AD_PSRAM)
...
#elif (TFLM_ARENA_LOCATION_F2M == NS_AD_SRAM)
...
uint8_t* ns_mem_model_ptr_f2m(void){
  ...
  return s_model_ptr;
#else
  return (uint8_t*)mut_model_f2m;  // const ok for reads; firmware places correctly
#endif
}
#endif
