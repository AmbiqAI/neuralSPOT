...
echo "Running all makes for Apollo330P"
for TC in "arm" "arm-none-eabi"; do
    for PL in "apollo330mP_evb"; do
        for TF in "helia_rt_v1_6_0" "tflm_nov_25"; do
            for AS in "R5.2.alpha.1.1"; do
                echo "Running: make PLATFORM=$PL AS_VERSION=$AS TF_VERSION=$TF"
                make -f Makefile PLATFORM=${PL} TOOLCHAIN=${TC} AS_VERSION=${AS} clean > /dev/null
